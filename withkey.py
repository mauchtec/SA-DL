from sadl import parse_bytes

def process_hex_string(hex_string, license_key):
    """Process hex string directly"""
    try:
        # Convert hex string to bytes
        raw_bytes = bytes.fromhex(hex_string)
        
        print(f"Converted hex to {len(raw_bytes)} bytes")
        
        # Parse the bytes (encrypted=True since this is raw license data)
        license_data = parse_bytes(raw_bytes, encrypted=True)
        
        if license_data:
            print("Successfully decoded license from hex data:")
            print(license_data)
        else:
            print("Failed to decode license data")
            
    except Exception as e:
        print(f"Error: {e}")

# Your hex data
hex_data = ""
# Your license key
license_key = ""

process_hex_string(hex_data, license_key)